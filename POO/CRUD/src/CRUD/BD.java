...
public class BD {
    ...
    public boolean conexcion(boolean Notificar){
        try{
            conectar = (Connection) DriverManager.getConnection(Cadena, user, password);
            if(Notificar) System.out.println("Conexion exitosa ");
            return true;
        }catch(Exception e){
            if(Notificar) System.out.println("No se encontro la base de datos,error: " + e);
        }
        return false;
    }
